...
export default function EditModalProjects({
  ...
  useEffect(() => {
    ...
    setFechaTerminacion(fechaTerminacionD ? fechaTerminacionD.substring(0,10) : '');
    setPresupuesto(presupuestoD);
    setOwner(ownerD);
    setEstadoProyecto(estadoProyectoD);
    setFaseProyecto(faseProyectoD);
  }, [nombreD, objetivosGeneralesD, objetivosEspecificosD, fechaInicioD, fechaTerminacionD, presupuestoD, ownerD, estadoProyectoD, faseProyectoD])

  const handleSubmit = (e) => {
    presupuesto = parseFloat(presupuesto)
    e.preventDefault();
    changeProject({ variables : { id: id, nombre: nombre, objetivosGenerales: objetivosGenerales, objetivosEspecificos: objetivosEspecificos,  fechaInicio: new Date(fechaInicio).toISOString(), fechaTerminacion: new Date(fechaTerminacion).toISOString(), presupuesto: presupuesto, owner: owner, estadoProyecto: estadoProyecto, faseProyecto: faseProyecto }});
    close()
  };

  const updateProject = gql`
  mutation updateProject($id: ID!, $nombre: String, $objetivosGenerales: String, $objetivosEspecificos: String, $fechaInicio: DateTime, $fechaTerminacion: DateTime, $estadoProyecto: String, $faseProyecto: String, $presupuesto: Float, $owner: ID) {
  updateProject(_id: $id, nombre: $nombre, objetivosGenerales: $objetivosGenerales, objetivosEspecificos: $objetivosEspecificos, fechaInicio: $fechaInicio, fechaTerminacion: $fechaTerminacion, estadoProyecto: $estadoProyecto, faseProyecto: $faseProyecto, presupuesto: $presupuesto, owner: $owner) {
    _id
    nombre
    objetivosGenerales
